...
import { updateJobAction } from "@/app/dashboard/jobs/actions";
...
export default async function EditJobPage({ params, searchParams }: EditJobPageProps) {
  ...
  const [{ data: companies }, { data: job }] = await Promise.all([
    ...
    supabase
      .from("jobs")
      .select(
        "id, company_id, title, role_category, location_country, location_city, remote_type, employment_type, seniority, salary_min, salary_max, salary_currency, summary, responsibilities, requirements, nice_to_haves, benefits, skills, screening_threshold",
      )
      .eq("id", id)
      .single(),
  ]);
